...
- [x] **Home update — walid's new assets (waves 10–12):** walid dropped new footage + backdrops into `public/` and said «استخدم كل اللي بعتو — من folder hero»:
  ...
  - **Hero — wave 13: walid's new `public/hero` footage + mouse-follow steering:** replaced `right.mp4`/`left.mp4` (deleted) with `public/hero/hero.mp4` (normal) + derived `hero-left.mp4` (reversed); the reduced-motion poster moved to `public/hero/frame-01.png` and is now the hero's `poster` too. Direction contract per walid: auto = normal rotation, moving the mouse RIGHT plays the REVERSED copy (hero-left.mp4), moving LEFT plays normal (hero.mp4), via mirrored-time reverse (same angle, no snap); ~1.2s idle returns to auto. Robust reveal via `loadeddata`/`playing` (not a blind timer). Old assets removed entirely (`public/360/`, `public/uploads/360/`).
