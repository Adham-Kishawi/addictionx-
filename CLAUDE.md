...
- **Model:** `Collection` model (migration `collections`): `slug` unique + `nameAr/nameEn` + `image` + `descriptionAr/descriptionEn` (slider captions) + `sortOrder` + `isActive`. **No FK between it and Product** — `Product.collection` stays a text slug (deliberate decision so nothing that reads the slug breaks).
- **Seed fix:** it used `prisma.category` for collections — **switched to `prisma.collection`**.
- **Reading:** `getCollections()` in `products-db.ts` (DB, falls back to the static `collections` array in `products.ts` if the DB is empty — must `map` because the constant is `readonly`). It now filters `where: { isActive: true }` — an admin can hide a collection (and its home-slider slide) without deleting it. Consumers: footer · home · catalog (filters) · product form (select shows `slug — name`).
- **Management:** `src/features/admin/collections-actions.ts` — `createCollection` (rejects duplicate slug via zod regex) · `updateCollection` (slug immutable once created — it's the text link products hold) · `deleteCollection` is **FREE**: it takes `{ moveTo }` (moves the collection's products to another collection first) or `{ deleteProducts }` (hard-deletes products with no orders; order-linked products are hidden — `isActive=false` — so order history survives). Passing neither while products exist returns `{ error: "NEED_CHOICE" }` and the UI dialog offers both options. Also `reorderCollection(slug, "up"|"down")` (renumbers `sortOrder` 10,20,30… in one transaction — robust against ties) and `toggleCollectionActive(slug)`. The `admin/collections` page shows every collection with **its perfumes' count and names** (aggregated by slug via Map) + the **slider controls** (`collection-slider-controls.tsx`: up/down reorder + eye show/hide). Form `collection-form.tsx` (create: image + AR/EN slider captions + slug derived automatically from the English name) + inline edit `collection-edit.tsx` + delete dialog `collection-delete.tsx`.
- **Collection slider controls:** `collection-slider-controls.tsx` renders per-collection reorder (↑/↓, disabled at first/last) + visibility (👁/👁🗨) buttons; actions live in `collections-actions.ts` and both revalidate `/`. These control the collections carousel **fallback** (see section 15c).
- **Dictionary keys:** `dict.admin.`.
...
- **Management:** `src/features/admin/slider-actions.ts` — `addSlide(productId, {image, captionAr, captionEn})` (appends at `position = max+10`) · `updateSlide` (cleans up the replaced stored upload) · `removeSlide` (cleans up its stored upload) · `reorderSlide(id, "up"|"down")` (renumbers 10,20,30…) · `toggleSlideActive(id)`. All `revalidatePath("/", "layout")` and require the **`products`** permission (same as Products/Best Sellers/Shipping).
- **Admin UI:** `admin/slider/page.tsx` (server) + `slider-manager.tsx` (client) — add a slide (product select with preview, custom image upload/URL, optional AR/EN captions), and per slide: thumbnail, reorder arrows, show/hide eye, inline edit (image + captions), remove. Warns when a slide's product is inactive (it won't render on the site). Nav item **Slider** sits right under **Products** (`admin-nav.tsx`, icon `Images`) and is gated by the `products` permission in `admin/layout.tsx`.
- **Dictionary keys:** `dict.admin.slider/sliderManage/sliderHint/sliderAdd/sliderAddHint/sliderSelectProduct/sliderNoProducts/sliderEmpty/sliderCustomImage/sliderCustomImageHint/sliderCaptionAr/sliderCaptionEn/sliderEdit/sliderRemove/sliderProductInactive`.
