...
- **Model:** `Collection` model (migration `collections`): `slug` unique + `nameAr/nameEn` + `image` + `descriptionAr/descriptionEn` (slider captions) + `sortOrder` + `isActive` + `showInHome` (home "Our Collections" section membership — independent from `isActive`). **No FK between it and Product** — `Product.collection` stays a text slug (deliberate decision so nothing that reads the slug breaks).
- **Seed fix:** it used `prisma.category` for collections — **switched to `prisma.collection`**.
- **Reading:** `getCollections(opts?)` in `products-db.ts` (DB, falls back to the static `collections` array in `products.ts` if the DB is empty — must `map` because the constant is `readonly`). Filters `where: { isActive: true }` — an admin can hide a collection from the site without deleting it. Pass `{ homeOnly: true }` to also filter `showInHome: true` (used by the home page for the "Our Collections" section only). Consumers: footer · home · catalog (filters) · product form (select shows `slug — name`).
- **Management:** `src/features/admin/collections-actions.ts` — `createCollection` (rejects duplicate slug via zod regex) · `updateCollection` (slug immutable once created — it's the text link products hold) · `deleteCollection` is **FREE**: it takes `{ moveTo }` (moves the collection's products to another collection first) or `{ deleteProducts }` (hard-deletes products with no orders; order-linked products are hidden — `isActive=false` — so order history survives). Passing neither while products exist returns `{ error: "NEED_CHOICE" }` and the UI dialog offers both options. Also `toggleCollectionActive(slug)` (site-wide hide). The `admin/collections` page shows every collection with **its perfumes' count and names** (aggregated by slug via Map) + a site-wide 👁 toggle (`collection-active-toggle.tsx`) + the **home-section manager** (see 15d). Form `collection-form.tsx` (create: image + AR/EN slider captions + slug derived automatically from the English name) + inline edit `collection-edit.tsx` + delete dialog `collection-delete.tsx`.
- **Dictionary keys:** `dict.admin.`.

### 15c. Home slider management from the dashboard (admin → Slider)

- **Model:** `HomeSlide` (table `home_slides`): `productId` FK → `Product` (Cascade delete) + `image` (custom slide image, `UploadedImage` URL or any URL — overrides the product's own image) + `captionAr`/`captionEn` (per-slide caption overrides) + `position` (slide order) + `isActive` (hide a slide without removing it).
- **Reading:** `getHomeSlides()` in `products-db.ts` returns active slides whose product is also active, ordered by `position asc`. The home page (`src/app/[lang]/(store)/page.tsx`) builds `carouselProducts` from these rows — overriding each product's `.image` with the slide's custom image and setting `slideCaptionsByProductId` (per-product captions beat collection `slideMeta` in `product-carousel.tsx`). **Fallback:** when no slides are configured (or all resolve to hidden products) the home page falls back to the old collection carousel (one product per collection, `collection.image` → `/slider/*.png` → product image), so the site never looks empty.
- **Management:** `src/features/admin/slider-actions.ts` — `addSlide(productId, {image, captionAr, captionEn})` (appends at `position = max+10`, returns `{id}`, rejects `DUPLICATE`/`INVALID_IMAGE`/`NOT_FOUND`) · `updateSlide` (cleans up the replaced stored upload) · `removeSlide` (cleans up its stored upload) · `reorderSlides(orderedIds)` (full-order commit renumbering 10,20,30…) · `toggleSlideActive(id)`. All `revalidatePath("/", "layout")` and require the **`products`** permission (same as Products/Best Sellers/Shipping).
- **Admin UI:** `admin/slider/page.tsx` (server) + `slider-manager.tsx` (client) + `slider-preview.tsx` — add a slide (searchable product picker, already-added disabled, custom image URL/upload, optional AR/EN captions), stats bar (visible/hidden/inactive), live storefront preview, drag to reorder + arrow buttons, show/hide eye, inline edit (image + captions), remove. Optimistic updates — no page reload; errors revert. Warns when a slide's product is inactive (it won't render on the site). Nav item **Slider** sits right under **Products** (`admin-nav.tsx`, icon `Images`) and is gated by the `products` permission in `admin/layout.tsx`.
- **Dictionary keys:** `dict.admin.`.

### 15d. Home "Our Collections" section management (admin → Collections)

- **Model:** `Collection.showInHome` (added via `prisma db push`) — decouples "appears in the home section" from `isActive` (site-wide visibility). Both must be true for the card to render on the homepage.
- **Reading:** the home page calls `getCollections({ homeOnly: true })` for the `CollectionShelf`; every other consumer (`/collections`, catalog filters, footer, sitemap) uses plain `getCollections()` (all active).
- **Management:** `collections-actions.ts` — `reorderCollections(orderedSlugs)` (full-order commit, renumbers 10,20,30…) · `addCollectionToHome(slug)` (sets `showInHome` + renumbers so it lands last) · `removeCollectionFromHome(slug)` (sets `showInHome=false` + renumbers the rest) · `updateCollectionSlider(slug, {image, descriptionAr, descriptionEn})` (inline card edit, validates the image URL). All require the `collections` permission.
- **Admin UI:** `admin/collections/page.tsx` (server) + `home-collections-manager.tsx` (client) + `collection-preview.tsx` — same experience as the product slider: stats bar (in section / not in section / off site), live storefront preview strip (collection image → fixed backdrop fallback, localized name + caption), searchable picker to add collections, drag to reorder + arrow buttons, remove from section, inline edit (card image + AR/EN captions). Optimistic — no page reload; errors revert. Per-collection site-wide 👁 toggle (`collection-active-toggle.tsx`) stays on each collection card below.
- **Dictionary keys:** `dict.admin.`.
...
- **WAVE 37 — programmatic Open Graph images (walid's TO DO item 4, OG part):** every page now ships a branded 1200×630 social card generated at runtime with `next/og` `ImageResponse` — deep black gradient + neon accent + the Heartbeat line + Cairo/Playfair. Shared shell `src/app/og-card.tsx` (fonts loaded once at module scope), three route files: `[lang]/opengraph-image.tsx` (global per-locale brand card), `[lang]/collections/[slug]/opengraph-image.tsx` (collection name), `[lang]/product/[slug]/opengraph-image.tsx` (fragrance name + price + its own `art.glow` accent). `[lang]/layout.tsx` gained `metadataBase` + localized `openGraph`. Two traps found: (1) the first font attempt used the **variable TTFs** (`Cairo[slnt,wght].ttf` / `PlayfairDisplay[wght].ttf`) which satori cannot parse → build crash `TypeError: Cannot read properties of undefined (reading '256')`; fixed with **static WOFFs** from fontsource jsDelivr (`assets/fonts/cairo-latin-400/700.woff`, `playfair-latin-700.woff`). (2) a **root-level** `src/app/opengraph-image.tsx` was never inherited by the `[lang]` pages (and the `/opengraph-image` single segment collides with the dynamic `[lang]`); the files live **inside `[lang]`** so `/en/opengraph-image` + `/ar/opengraph-image` resolve. Verified: 200 image/png (PNG magic `89 50 4E 47`), og:image + twitter:image present on /en /ar product + collection pages, live on Vercel.
- **WAVE 38 — per-page SEO metadata + Product JSON-LD (closed the REPORT-full-review gap):** product pages (`[lang]/product/[slug]/page.tsx`) now export `generateMetadata` → localized title/description, absolute `canonical` (resolved from `metadataBase`), `openGraph` (title/description/locale/url), and render a `Product` JSON-LD script: name, description, brand (ADDICTIONX), `Offer` (price in EGP from piasters, `priceCurrency`, `InStock`/`OutOfStock` availability) + `aggregateRating` (rating/reviewCount when >0); the JSON-LD `image` points at the generated OG route so shares always carry a real image. Collection pages got the same metadata (localized name + canonical + OG). Verified locally (prod server) and live: `Midnight Noir | ADDICTIONX | Feel the Rush`, canonical `https://addictionx.vercel.app/en/product/midnight-noir`, Arabic `منتصف الليل | ...`, JSON-LD present.
- **FONTS — self-hosted, Google-Fonts dependency removed:** `next/font/google` (`Cairo`, `Playfair_Display` in `[lang]/layout.tsx`) caused flaky Turbopack/dev/build failures (`Can't resolve '@vercel/turbopack-next/internal/font/google/font'`, aggravated by a corrupted `.next` after killed dev servers) plus a network dependency on Google Fonts at build time. Replaced with **`@fontsource/cairo` + `@fontsource/playfair-display`** CSS imports (weights 300–700; the per-weight CSS includes all subsets via `unicode-range` — arabic included). `globals.css` `:root, .dark` now defines `--font-cairo` / `--font-playfair` (the vars `next/font` used to inject), `@theme inline` mappings unchanged, no `cairo.variable` classes needed. Verified: no `fonts.googleapis.com` link in HTML, 15 Cairo `@font-face` rules + woff2 in the CSS bundle, clean tsc + eslint + build.
- **WAVE 39 — SEO infrastructure (sitemap / robots / hreflang / structured data):** `src/app/sitemap.ts` emits `/sitemap.xml` with **28 URLs** (en+ar × home/catalog/collections + all 3 collections + all 8 products), each with `lastModified` + per-locale `alternates.languages` (hreflang). `src/app/robots.ts` emits `/robots.txt`: allow all, disallow `/en/admin/` + `/ar/admin/`, points at the sitemap. Product + collection pages now return `alternates.languages` (`en`/`ar` link rel=alternate — rendered by React as `hrefLang`). Homepage (`[lang]/page.tsx`) gained an `Organization` + `WebSite` JSON-LD with a `SearchAction`. New shared `src/lib/site-url.ts` (`NEXT_PUBLIC_SITE_URL ?? https://addictionx.vercel.app`) now feeds `metadataBase`, product JSON-LD/canonical, sitemap + robots. Verified locally (prod build) and live: sitemap 200/28 urls, robots 200, hreflang present on product pages, clean tsc + eslint + build.
