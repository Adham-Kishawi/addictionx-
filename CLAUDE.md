...
- **Wave 13 (walid's direction):** new turntable footage `public/hero/hero.mp4` (1280×720 · 24fps · ~10s, full 360° turn **right→left**) replaces the old pair — **old assets deleted entirely**: `right.mp4`, `left.mp4`, `public/360/perfume-360.mp4`, and `public/uploads/360/` (its only survivor, the reduced-motion poster, moved to `public/hero/frame-01.png`).
- **Assets:** `public/hero/hero.mp4` (forward, right→left) + derived `public/hero/hero-left.mp4` (reversed, left→right). Regenerate the reverse with `ffmpeg -i hero.mp4 -vf "trim=start_frame=1,reverse" hero-left.mp4`. `TurntableVideo` plays `/hero/hero.mp4` ↔ `/hero/hero-left.mp4`; the showcase poster is `/hero/frame-01.png`.
- **Idle = auto 360°:** while the cursor is outside the hero (or stopped) the two videos alternate on `ended` (hero.mp4 right→left ↔ hero-left.mp4 left→right) — a continuous no-seam ping-pong, one full 360° turn each way.
- **Mouse-follow (`interactive` prop — hero only):** a `window` mousemove listener gated to the hero's bounding rect steers the turn — moving right plays hero-left.mp4 (bottle turns right), moving left plays hero.mp4 (bottle turns left). Direction switches mid-turn use a **mirrored-time swap** (`duration − currentTime`) so the bottle keeps its exact angle and simply reverses — no snap. ~1.2s after the mouse stops (or on leaving the hero) it eases back to the auto spin. `prefers-reduced-motion` still shows the static frame.
...
- [x] **Home update — walid's new assets (waves 10–12):** walid dropped new footage + backdrops into `public/` and said «استخدم كل اللي بعتو — من folder hero»:
  ...
  - **Hero — wave 13: walid's new `public/hero` footage + mouse-follow steering:** replaced `right.mp4`/`left.mp4` (deleted) with `public/hero/hero.mp4` (right→left) + derived `hero-left.mp4`; the reduced-motion poster moved to `public/hero/frame-01.png`. `TurntableVideo` gained the `interactive` prop (hero only): while the cursor is away it auto-spins its 360° ping-pong; moving over the hero steers the turn right/left via a mirrored-time reverse (same angle, no snap), returning to the auto spin after ~1.2s idle. Old assets removed entirely (`public/360/`, `public/uploads/360/`).
  - Verified: clean tsc + eslint (waves 10–13).
